...
std::vector<Point> smoothPath(const std::vector<Point>& path, int numInterpolatedPoints = 100) {
    if (path.size() < 3) {
    ...
    // Create a vector for the cumulative distance 't' for each point
    std::vector<double> t(path.size(), 0.0);
    for (size_t i = 1; i < path.size(); ++i) {
        t[i] = t[i - 1] + cv::norm(path[i] - path[i - 1]);
    }

    // Use OpenCV's interpolation method
    cv::Mat xPoints(path.size(), 1, CV_64F);
    cv::Mat yPoints(path.size(), 1, CV_64F);

    for (size_t i = 0; i < path.size(); ++i) {
        xPoints.at<double>(i, 0) = path[i].x;
        yPoints.at<double>(i, 0) = path[i].y;
    }

    cv::Mat smoothedX, smoothedY;
    cv::resize(xPoints, smoothedX, cv::Size(1, numInterpolatedPoints), 0, 0, cv::INTER_CUBIC);
    cv::resize(yPoints, smoothedY, cv::Size(1, numInterpolatedPoints), 0, 0, cv::INTER_CUBIC);

    for (int i = 0; i < numInterpolatedPoints; ++i) {
        smoothedPath.emplace_back(static_cast<int>(smoothedX.at<double>(i, 0)), static_cast<int>(smoothedY.at<double>(i, 0)));
    }
...
void drawPathPlanning(Mat& display, const Point& lidarPosition, const Point& destination, const std::vector<std::vector<Point>>& bounding_boxes) {
    ...
    std::vector<Point> smoothed_path = smoothPath(shortest_path);

    // Draw the smoothed path
    if (!smoothed_path.empty()) {
        for (size_t i = 1; i < smoothed_path.size(); ++i) {
            line(display, smoothed_path[i - 1], smoothed_path[i], Scalar(0, 255, 255), 4); // Draw the smooth path in yellow
        }
    } else {
