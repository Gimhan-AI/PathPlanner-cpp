...
// Define delay function for Linux/Unix
...
// Function to clear the console screen for Linux/Unix
void clearScreen() {
...
// Function to draw Lidar points and bounding boxes
void drawLidarPointsAndBoundingBoxes(Mat& display, const Point& lidarPosition, const std::vector<std::pair<float, float>>& points, float fixed_length_extension, float fixed_width_extension) {
    ...
    for (const auto& cluster : clusters) {
        ...
        float distance_to_center = sqrt(pow(center.x - lidarPosition.x, 2) + pow(center.y - lidarPosition.y, 2)) * 10 / 1000.0;
        char distance_text[50];
        sprintf(distance_text, "Distance: %.2f m", distance_to_center);
        putText(display, distance_text, center, FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255, 255, 255), 1);
    }
...
// Function to handle LiDAR operations in a thread
void lidarThread(ILidarDriver *drv) {
...
int main(int argc, const char *argv[]) {
    ...
    do {
        ...
        std::thread lidar_thread(lidarThread, drv);
