...
const float AREA_THRESHOLD = 14000.0;
...
std::vector<std::string> load_class_list() {
    ...
    std::ifstream ifs("/home/vegaai/sensorfusion/vegaav/object_detection/classes.txt");
    std::string line;
...
void load_net(cv::dnn::Net& net) {
    auto result = cv::dnn::readNet("/home/vegaai/sensorfusion/vegaav/object_detection/yolov5s.onnx");
...
void drawPathPlanning(Mat& display, const Point& lidarPosition, const Point& destination, const std::vector<std::vector<Point>>& bounding_boxes) {
    ...
    if (direct_path_clear) {
        line(display, lidarPosition, destination, Scalar(0, 255, 0), 4);
    } else {
        std::vector<Point> graph_points;
        graph_points.push_back(lidarPosition);
        graph_points.push_back(destination);

        for (const auto& box : bounding_boxes) {
            for (const auto& corner : box) {
                bool valid = true;
                for (const auto& other_box : bounding_boxes) {
                    if (line_intersects_box(lidarPosition, corner, other_box)) {
                        valid = false;
                        break;
                    }
                }
                if (valid) {
                    graph_points.push_back(corner);
                    line(display, lidarPosition, corner, Scalar(128, 0, 128), 2);
                }
            }
        }
        ...
        for (const auto& box : bounding_boxes) {
            for (const auto& corner : box) {
                bool valid = true;
                for (const auto& other_box : bounding_boxes) {
                    if (line_intersects_box(corner, destination, other_box)) {
                        valid = false;
                        break;
                    }
                }
                if (valid) {
                    line(display, corner, destination, Scalar(255, 165, 0), 2);
                }
            }
        }
    }
...
void drawLidarPointsAndBoundingBoxes(Mat& display, const Point& lidarPosition, const Point& destination, const std::vector<std::pair<float, float>>& points, float fixed_length_extension, float fixed_width_extension) {
    ...
    for (const auto& cluster : clusters) {
        ...
        bounding_boxes.push_back(calculate_corners(Point(boundingBox.x + boundingBox.width / 2, boundingBox.y + boundingBox.height / 2), boundingBox.width));
    }
